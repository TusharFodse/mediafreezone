...
const PinPage = ({ user }) => {
    ...
    //   try {
    //     console.log(`Downloading from: /api/pins/download/${pin._id}`);
    //     // console.log("Download request received for pin ID:", req.params.id);
    //     const response = await axios.get(`/api/pins/download/${pin._id}`, {
    //       responseType: "blob",
    //     });
    ...
    //     link.href = url;
    //     link.setAttribute("download", `${pin.title}.jpg`);
    ...
    const handleDownload = async () => {
      try {
          const response = await fetch(pin.image.url); // Fetch the image
          const blob = await response.blob(); // Create a Blob from the image
          const url = window.URL.createObjectURL(blob); // Create an object URL

          const link = document.createElement('a'); // Create an anchor element
          link.href = url;
          link.setAttribute('download', `${pin.title}.jpg`); // Set the desired filename
  ...
  return (
    <div>
      {pin && (
        <div className="flex flex-col items-center p-4 min-h-screen bg-gradient-to-r from-purple-500 via-pink-500 to-red-500">
            ...
            <div className="w-full md:w-1/2 p-6 flex flex-col">
              ...
              <div className="overflow-y-auto h-64">
                {pin.Comment && pin.Comment.length > 0 ? (
                  ...
                  <p>Be the first one to add the comment</p>
                )}
              </div>
            </div>
          </div>
